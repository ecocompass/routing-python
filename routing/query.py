...
class Query:
    ...
    def bus_walking_route(self, start, end):
        print(f"[Computing bus-walking routes from {start} to {end}]\n")
        ...
        nearest_stops_start = routing_core.get_nearest_nodes(start, self.bus_tree, self.bus_nodes)
        nearest_stops_end = routing_core.get_nearest_nodes(end, self.bus_tree, self.bus_nodes)

        transit_routes = routing_core.get_transit_routes(nearest_stops_start, nearest_stops_end, self.transit_map, "bus")
        ...
        for transit_route in transit_routes:
            ...
            transit_time = (distance / common.average_speeds['bus']) * 60

            # get walking route to the start stop
            road_node_start_stop = routing_core.get_nearest_nodes(
                (
                    self.transit_map["bus_stops"][start_stop]["lat"],
                    self.transit_map["bus_stops"][start_stop]["lon"]
            ...
            road_node_end_stop = routing_core.get_nearest_nodes(
                (
                    self.transit_map["bus_stops"][end_stop]["lat"],
                    self.transit_map["bus_stops"][end_stop]["lon"]
                ), self.road_tree, self.road_nodes, 2
            )[0]
            ...
            # TODO: currently basic method to check time delta betweeen walking and bus arrival
            candidate_time = walking_time + delta_wait_walking
            total_time = walking_time_end + transit_time + walking_time_end

            # print(
            #     f"*Transit route {route_label}: {self.transit_map['bus_stops'][start_stop]['name']} -> {self.transit_map['bus_stops'][end_stop]['name']}")
        ...
        return {
            ...
            "end": end,
            "bus": {
                "start_stop": (self.transit_map['bus_stops'][start_stop]['lat'], self.transit_map['bus_stops'][start_stop]['lon']),
                "end_stop": (self.transit_map['bus_stops'][end_stop]['lat'], self.transit_map['bus_stops'][end_stop]['lon']),
                "label": route_label,
                "distance": distance,
                "trace": trace,
                "wait_time": wait_time,
